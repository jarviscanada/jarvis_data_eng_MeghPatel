...
@dp.table(
    name="jrvs_databricks.silver.msft_silver",
    comment="Cleaned and formatted daily stock data for MSFT"
)
@dp.expect_or_drop("valid_close_price", "close_price > 0")
@dp.expect_or_drop("valid_volume", "volume > 0")
def msft_silver():
    return (
        spark.readStream.table("jrvs_databricks.bronze.msft_bronze")
        .select(
            F.col("symbol"),
