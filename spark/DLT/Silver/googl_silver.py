...
@dp.table(
    name="jrvs_databricks.silver.googl_silver",
    comment="Cleaned and formatted daily stock data for GOOGL"
)
@dp.expect_or_drop("valid_close_price", "close_price > 0")
@dp.expect_or_drop("valid_volume", "volume > 0")
def googl_silver():
    return (
        spark.readStream.table("jrvs_databricks.bronze.googl_bronze")
        .select(
            F.col("symbol"),
